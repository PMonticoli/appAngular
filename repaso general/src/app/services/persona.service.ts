...
export class PersonaService {
  ...
  nombreExiste(value: string): Observable<boolean> {
    return this.http.get<Persona[]>(this.API_URL).pipe(
      map(z => z.some((persona) => persona.nombre == value)),
    );
  }

  apellidoExiste(value: string): Observable<boolean> {
    return this.http.get<Persona[]>(this.API_URL).pipe(
      map(z => z.some((persona) => persona.apellido == value)),
    );
  }
}
